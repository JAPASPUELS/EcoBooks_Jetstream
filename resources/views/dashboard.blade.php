            ...
            <div class="flex gap-6">
                ...
                <div class="w-1/3 ">
                    <div id="updateCountBox "
                        class="flex flex-col h-56 items-center justify-center py-8 px-4 mb-2 bg-gray-100 rounded-lg">
                        <span id="updateCount" class="text-8xl font-bold text-green-500 mb-2">0</span>
                        <span class="text-xs font-semibold text-gray-700">TOTAL VENTAS ($)</span>
                    </div>
                </div>
                <div class="w-1/3">
                    <div id="insertCountBox"
                        ...
                        <span class="text-xs font-semibold text-gray-700">NUMERO DE VENTAS</span>
                    </div>
                </div>

                <div class="w-1/7 flex flex-col">
                    <div id="deleteCountBox"
                        class="flex flex-col items-start justify-left  px-4 mb-2 bg-gray-100 rounded-lg">
                        <span id="deleteCount" class="text-8xl font-bold text-blue-500 mb-2 hidden">0</span>
                        <!-- Date Range Picker -->
                        <div id="date-range-picker" class="flex flex-col items-center mb-5">
                            <div class="relative mb-4">
                                <label for="datepicker-range-start" class="block text-gray-700 text-sm font-bold mb-2">Fecha Inicio</label>
                                <input id="datepicker-range-start" name="start" type="date"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 pr-4  placeholder-gray-400 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            ...
                            <div class="">
                                <button id="filterButton" class="w-48  bg-blue-500 hover:bg-blue-700 text-white py-2 px-4 rounded-lg">
                                    Filtrar
                                </button>
                                 <button id="openModalButton"
                                    class="bg-orange-500 hover:bg-orange-700 text-white py-2 px-4 rounded-lg ml-2 hidden">Datos</button>
                
                            </div>
                        </div>
                    </div>
                    <!-- Puedes agregar más widgets aquí, y se apilarán verticalmente -->
                </div>
 ...
 --}}
            ...
            <div id="dataModal" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75 hidden ">
                <div class="bg-white rounded-lg shadow-lg p-6 w-3/5">
                    ...
                    <button id="reportButton" class="bg-green-500 hover:bg-green-700 text-white py-2 px-3 rounded-lg my-2">
                        Reporte</button>
    ...
    {{-- <div class="flex gap-6 "> --}}
                <!-- Tabla de auditoría -->
                <div class=" h-1/3 bg-white rounded-lg shadow-md border border-gray-200">
                    <table id="dataTable" class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col"
                                    class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Subtotal
                                </th>
                                <th scope="col"
        ...
        <div class="hidden w-1/4 bg-white rounded-lg shadow-md border border-gray-200 ">
        ...
        <div class="hidden w-1/5 bg-white rounded-lg shadow-md border border-gray-200">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const countElements = {
                'NUMERODEVENTAS': document.getElementById('insertCount'),
                'SUMATOTALVENTAS': document.getElementById('updateCount'),
                'SUMATOTALVENTAS2': document.getElementById('deleteCount')
            };
            ...
            function fillAuditoriaTable(auditoriaData) {
                ...
                auditoriaData.data.data.forEach(entry => {
                    ...
                    row.innerHTML = `
                <td class="border border-gray-300 px-4 py-2">${entry.vent_numero}</td>
                <td class="border border-gray-300 px-4 py-2">${entry.vent_fecha}</td>
                <td class="border border-gray-300 px-4 py-2">${entry.vent_subtotal}</td>
                <td class="border border-gray-300 px-4 py-2">${entry.vent_total}</td>
            `;
                ...
                console.log(auditoriaData.data)
                // Establecer las clases de Tailwind para los estilos
                const baseLinkClasses = 'inline-block rounded px-3 py-1 mx-1';

                if (auditoriaData.data.current_page > 1) {
                    paginationLinks.innerHTML +=
                        `<a href="#" class="${baseLinkClasses} pagination-link mt-1 bg-gray-200 hover:bg-gray-300 text-gray-700 hover:text-gray-900" data-page="${auditoriaData.data.current_page - 1}">&laquo; Previo</a>`;
                }

                for (let i = 1; i <= auditoriaData.data.last_page; i++) {
                    paginationLinks.innerHTML +=
                        `<a href="#" class="${baseLinkClasses} pagination-link mt-1 ${i === auditoriaData.data.current_page ? 'active bg-blue-500 text-white' : 'bg-gray-200 hover:bg-gray-300 text-gray-700 hover:text-gray-900'}" data-page="${i}">${i}</a>`;
                }

                if (auditoriaData.data.current_page < auditoriaData.data.last_page) {
                    paginationLinks.innerHTML +=
                        `<a href="#" class="${baseLinkClasses} pagination-link mt-1 bg-gray-200 hover:bg-gray-300 text-gray-700 hover:text-gray-900" data-page="${auditoriaData.data.current_page + 1}">Siguiente &raquo;</a>`;
                }
